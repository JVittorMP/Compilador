...
namespace lex {
    ...
    static std::string value(pattern p) {
        switch(p) {
    ...
    static std::string name(pattern p) {
        switch(p) {
            case pattern::OPERATOR: return "OPERATOR";
            case pattern::KEYWORD: return "KEYWORD";
            case pattern::PUNCTUATION: return "PUNCTUATION";
    ...
    static bool terminal(pattern p) {
        switch(p) {
    ...
    static pattern enumeration(const std::string & s) {
    ...
    public:
        ...
        token() = default;
        explicit token(enum pattern p): pattern(p), value(lex::value(p)) {}
        token(const enum pattern p, std::string value, unsigned linha): pattern(p), value(std::move(value)), linha(linha) {}

        bool operator==(const token & token) const {
            return std::tie(this->pattern, this->value) == std::tie(token.pattern, token.value);
        }
    };

    inline void showTokens(const std::vector<token> & tokens) {
        for(auto & [k, v, l] : tokens) {
            std::cout << l << ": [" << name(k) << ", " << v << "]" << std::endl;
        }
    }
