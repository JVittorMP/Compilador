...
double resolveExpresion(ast::Node02* node) {
    return 0;
}
...
ast::Node02* pop(std::stack<ast::Node02*> & nodes) {
    ast::Node02* node = nodes.top();
    nodes.pop();
    return node;
}
ast::Node02* buildTree(const std::deque<ast::Node02*>& entries) {
    std::stack<ast::Node02*> nodes;
    ...
    for(const auto & nd : entries) {
        if(isNumber(nd->value) || !isOperand(nd->value)) {
            ...
            while(!ops.empty() && priority(ops.top(), nd->value)) {
                auto node = ast::Node02::pointer(lex::Type(lex::Type::pattern::OPERATOR), pop(ops));
...
std::deque<ast::Node02*> getExpression(std::deque<ast::Node02*> v, ast::Node02* & cur);
void simplifyExpression(ast::Node02* node, std::deque<ast::Node02*> & exp);


void explore(ast::Node02* node, int i = 0) {
...
void exploreT(ast::Node02* node, int i = 0) {
    for(int j = 0; j < size(node->tokens); j++) {
        auto n = node->tokens[j];
...
void explore02(ast::Node02* node, int i = 0) {
    std::cout << std::endl << tab(i) << "[(0:0):" << node->type.asString() << ", " << node->value << "]";
    exploreT(node, i + 1);
}

void getVars(std::deque<ast::Node02*> & v, ast::Node02* cur) {
...
void getDeclarations(std::deque<ast::Node02*> & dc, ast::Node02* cur) {
    if(cur->tokens.empty()) return;
    for(auto n : cur->tokens) {
        if(n->type.pattern == lex::Type::pattern::VAR) {
            for(auto b : n->tokens) {
                if(b->type.pattern == lex::Type::pattern::VARS) {
                    std::deque<ast::Node02*> vars;
                    getVars(vars, b);
                    b->tokens = vars;
                }
            }
...
void getCmds(std::deque<ast::Node02*> & cmds, ast::Node02* cur) {
    if(cur->tokens.empty()) return;
    for(auto & n : cur->tokens) {
...
void avaluateNegNum(ast::Node02* & cur) {
    if(cur->tokens.empty()) return;
...
ast::Node02* changeExpressionNode(ast::Node02* ex) {
    std::deque<ast::Node02*> v;
    avaluateNegNum(ex);
    ast::Node02* exCpy = ex;
...
void orderAtribution(ast::Node02* & node) {
    ast::Node02* & id = node->tokens[0];
    ast::Node02* & ri = node->tokens[1];
    ast::Node02* & op = ri->tokens[0];
    ast::Node02* & ex = ri->tokens[1]->tokens[0];

    if(ex->type.pattern != lex::Type::pattern::INPUT) ex = changeExpressionNode(ex);

    auto* novo = ast::Node02::pointer(op->type, op->value);
...
void orderConditionals(ast::Node02* & node) {
    ast::Node02* & cond = node->tokens[1];
    ast::Node02* & ex1 = cond->tokens[0];
    ast::Node02* & ex2 = cond->tokens[2];

    ex1 = changeExpressionNode(ex1);
    ex2 = changeExpressionNode(ex2);
}

void orderOutput(ast::Node02* & node) {
    std::deque<ast::Node02*> exp;
    ast::Node02* & ex = node->tokens[1];
    simplifyExpression(ex, exp);
    ex->tokens.clear();
    ex->tokens.push_back(buildTree(exp));
}

void getArguments(ast::Node02* & node, std::deque<ast::Node02*> & v) {
    for(auto nd : node->tokens) {
...
ast::Node02* orderArguments(ast::Node02* & node) {
    std::deque<ast::Node02*> v;
    getArguments(node, v);
    node->tokens = v;
    return node;
...
void filterCmds(ast::Node02* & node) {
    for(auto & n : node->tokens) {
        ast::Node02* cur = n->tokens.front();
...
void getParams(std::deque<ast::Node02*> & v, ast::Node02* cur) {
    if(cur->tokens.empty()) return;
    auto* last = cur->tokens.back();

    auto* param = ast::Node02::pointer(lex::Type(lex::Type::pattern::PARAM));
...
void compressSignature(ast::Node02* & node) {
    std::deque<ast::Node02*> v;
    ast::Node02* & params = node->tokens[2];
    getParams(v, params);
    params->tokens = v;
}

void compressReturn(ast::Node02* & node) {
    std::deque<ast::Node02*> v;
    ast::Node02* & exp = node->tokens[1];
    exp = changeExpressionNode(exp);
}

void getMethod(ast::Node02* node, std::deque<ast::Node02*> & v) {
    auto* signature =  ast::Node02::pointer(lex::Type(lex::Type::pattern::SIGNATURE));
    auto* retorno = ast::Node02::pointer(lex::Type(lex::Type::pattern::RETURN));
    for(auto & nd : node->tokens) {
...
void compressSubTree(const std::string & s, ast::Node02* node, std::deque<ast::Node02*> & v) {
...
void compressStructure(ast::Node02* & node) {
...
void eraseEmptyProductions(ast::Node02* & node) {
    for(int i = 0; i < size(node->tokens); i++) {
        ast::Node02* n = node->tokens[i];
...
void removeInit(ast::Node02* & root) {
...
void compressTest(ast::Node02* & node) {
    for(auto & n : node->tokens) {
        if(n->tokens.empty()) continue;
...
ast::Node02* compress(ast::Node02* & root) {
...
ast::Node02* buildAST(ast::Node02* root) {
