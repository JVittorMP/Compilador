...
void exploreTNode02(ast::Node02* node, int i = 0) {
    for(int j = 0; j < size(node->tokens); j++) {
        ...
        exploreTNode02(n, i + 1);
    }
}

void exploreNode02(ast::Node02* node, int i = 0) {
    std::cout << std::endl << tab(i) << "[(0:0):" << node->type.asString() << ", " << node->value << "]";
    exploreTNode02(node, i + 1);
}

int PROG(ast::Node02* node, std::deque<lex::token02>::iterator & tokens);
int INIT(ast::Node02* node, std::deque<lex::token02>::iterator & tokens);
int METODO(ast::Node02* node, std::deque<lex::token02>::iterator & tokens);
int PARAMS(ast::Node02* node, std::deque<lex::token02>::iterator & tokens);
int MAIS_PARAMS(ast::Node02* node, std::deque<lex::token02>::iterator & tokens);
int DC(ast::Node02* node, std::deque<lex::token02>::iterator & tokens);
int MAIS_DC(ast::Node02* node, std::deque<lex::token02>::iterator & tokens);
int VAR(ast::Node02* node, std::deque<lex::token02>::iterator & tokens);
int VARS(ast::Node02* node, std::deque<lex::token02>::iterator & tokens);
int MAIS_VAR(ast::Node02* node, std::deque<lex::token02>::iterator & tokens);
int TIPO(ast::Node02* node, std::deque<lex::token02>::iterator & tokens);
int CMDS(ast::Node02* node, std::deque<lex::token02>::iterator & tokens);
int CONDITIONAL_CMD(ast::Node02* node, std::deque<lex::token02>::iterator & tokens);
int CMD(ast::Node02* node, std::deque<lex::token02>::iterator & tokens);
int PFALSA(ast::Node02* node, std::deque<lex::token02>::iterator & tokens);
int RESTO_IDENT(ast::Node02* node, std::deque<lex::token02>::iterator & tokens);
int LISTA_ARG(ast::Node02* node, std::deque<lex::token02>::iterator & tokens);
int ARGUMENTOS(ast::Node02* node, std::deque<lex::token02>::iterator & tokens);
int MAIS_IDENT(ast::Node02* node, std::deque<lex::token02>::iterator & tokens);
int EXP_IDENT(ast::Node02* node, std::deque<lex::token02>::iterator & tokens);
int CONDICAO(ast::Node02* node, std::deque<lex::token02>::iterator & tokens);
int RELACAO(ast::Node02* node, std::deque<lex::token02>::iterator & tokens);
int EXPRESSAO(ast::Node02* node, std::deque<lex::token02>::iterator & tokens);
int TERMO(ast::Node02* node, std::deque<lex::token02>::iterator & tokens);
int OP_UN(ast::Node02* node, std::deque<lex::token02>::iterator & tokens);
int FATOR(ast::Node02* node, std::deque<lex::token02>::iterator & tokens);
int OUTROS_TERMOS(ast::Node02* node, std::deque<lex::token02>::iterator & tokens);
int OP_AD(ast::Node02* node, std::deque<lex::token02>::iterator & tokens);
int MAIS_FATORES(ast::Node02* node, std::deque<lex::token02>::iterator & tokens);
int OP_MUL(ast::Node02* node, std::deque<lex::token02>::iterator & tokens);

ast::Node02* parse(ast::Node02* node, std::deque<lex::token02>::iterator & tokens);

int findNonTerminal(enum lex::Type::pattern p, ast::Node02* node, std::deque<lex::token02>::iterator & token) {
    switch (p) {
...
int isProductionValid(ast::Node02* node, std::deque<lex::token02>::iterator & token, std::deque<lex::token02> & lexemas) {
    for(auto & lexema : lexemas) {
        ...
        if(lexema.type.status()) {
            ...
            node->tokens.push_back(ast::Node02::pointer((*token)));
            token++;
        } else {
            auto* novo = ast::Node02::pointer(lexema.type, lexema.value);
...
ast::Node02* parse(std::deque<lex::token02>::iterator & token) {
    auto* root = ast::Node02::pointer(lex::Type(lex::Type::pattern::PROG));
...
int PROG(ast::Node02* node, std::deque<lex::token02>::iterator & token) {
    std::deque<lex::token02> lexemas; 
...
int INIT(ast::Node02* node, std::deque<lex::token02>::iterator & token) {
    std::deque<lex::token02> lexemas; 
    lexemas.emplace_back(lex::Type(lex::Type::pattern::PUBLIC));
...
int METODO(ast::Node02* node, std::deque<lex::token02>::iterator &token) {
    std::deque<lex::token02> lexemas; 
    lexemas.emplace_back(lex::Type(lex::Type::pattern::PUBLIC));
    lexemas.emplace_back(lex::Type(lex::Type::pattern::STATIC));
    ...
    node->tokens.push_back(ast::Node02::pointer(lex::Type{lex::Type::pattern::VAZIO}));
    return SUCCESS;
}

int PARAMS(ast::Node02* node, std::deque<lex::token02>::iterator &token) {
    std::deque<lex::token02> lexemas; 
    lexemas.emplace_back(lex::Type(lex::Type::pattern::TIPO));
    lexemas.emplace_back(lex::Type(lex::Type::pattern::IDENTIFIER));
...
int MAIS_PARAMS(ast::Node02* node, std::deque<lex::token02>::iterator &token) {
    std::deque<lex::token02> lexemas; 
...
int DC(ast::Node02* node, std::deque<lex::token02>::iterator &token) {
    std::deque<lex::token02> lexemas; 
...
int MAIS_DC(ast::Node02* node, std::deque<lex::token02>::iterator &token) {
    std::deque<lex::token02> lexemas; 
    lexemas.emplace_back(lex::Type(lex::Type::pattern::PONTO_VIRGULA));
    lexemas.emplace_back(lex::Type(lex::Type::pattern::DC));
...
int VAR(ast::Node02* node, std::deque<lex::token02>::iterator &token) {
    std::deque<lex::token02> lexemas; 
    lexemas.emplace_back(lex::Type(lex::Type::pattern::TIPO));
...
int VARS(ast::Node02* node, std::deque<lex::token02>::iterator &token) {
    std::deque<lex::token02> lexemas; 
    lexemas.emplace_back(lex::Type(lex::Type::pattern::IDENTIFIER));
...
int MAIS_VAR(ast::Node02* node, std::deque<lex::token02>::iterator &token) {
    std::deque<lex::token02> lexemas; 
    lexemas.emplace_back(lex::Type(lex::Type::pattern::VIRGULA));
    lexemas.emplace_back(lex::Type(lex::Type::pattern::VARS));
    if((*token).value == lex::Type::asValue(lex::Type::pattern::VIRGULA)) {
...
int TIPO(ast::Node02* node, std::deque<lex::token02>::iterator &token) {
    std::deque<lex::token02> lexemas; 
...
int CMDS(ast::Node02* node, std::deque<lex::token02>::iterator &token) {
    std::deque<lex::token02> lexemas; 
...
int CONDITIONAL_CMD(ast::Node02* node, std::deque<lex::token02>::iterator &token) {
    std::deque<lex::token02> lexemas; 
...
int CMD(ast::Node02* node, std::deque<lex::token02>::iterator &token) {
    std::deque<lex::token02> lexemas; 
...
int PFALSA(ast::Node02* node, std::deque<lex::token02>::iterator &token) {
    std::deque<lex::token02> lexemas; 
...
int RESTO_IDENT(ast::Node02* node, std::deque<lex::token02>::iterator &token) {
    std::deque<lex::token02> lexemas; 
...
int LISTA_ARG(ast::Node02* node, std::deque<lex::token02>::iterator &token) {
    std::deque<lex::token02> lexemas; 
...
int ARGUMENTOS(ast::Node02* node, std::deque<lex::token02>::iterator & token) {
    std::deque<lex::token02> lexemas; 
    lexemas.emplace_back(lex::Type(lex::Type::pattern::IDENTIFIER));
...
int MAIS_IDENT(ast::Node02* node, std::deque<lex::token02>::iterator &token) {
    std::deque<lex::token02> lexemas; 
    lexemas.emplace_back(lex::Type(lex::Type::pattern::VIRGULA));
    lexemas.emplace_back(lex::Type(lex::Type::pattern::ARGUMENTOS));
    if((*token).value == lex::Type::asValue(lex::Type::pattern::VIRGULA)) {
...
int EXP_IDENT(ast::Node02* node, std::deque<lex::token02>::iterator &token) {
    std::deque<lex::token02> lexemas; 
    lexemas.emplace_back(lex::Type(lex::Type::pattern::EXPRESSAO));
    if( (*token).type == lex::Type(lex::Type::pattern::IDENTIFIER) ||
...
int CONDICAO(ast::Node02* node, std::deque<lex::token02>::iterator &token) {
    std::deque<lex::token02> lexemas; 
    lexemas.emplace_back(lex::Type(lex::Type::pattern::EXPRESSAO));
...
int RELACAO(ast::Node02* node, std::deque<lex::token02>::iterator &token) {
...
int EXPRESSAO(ast::Node02* node, std::deque<lex::token02>::iterator &token) {
    std::deque<lex::token02> lexemas; 
...
int TERMO(ast::Node02* node, std::deque<lex::token02>::iterator &token) {
    std::deque<lex::token02> lexemas; 
...
int OP_UN(ast::Node02* node, std::deque<lex::token02>::iterator &token) {
...
int FATOR(ast::Node02* node, std::deque<lex::token02>::iterator &token) {
...
int OUTROS_TERMOS(ast::Node02* node, std::deque<lex::token02>::iterator &token) {
    std::deque<lex::token02> lexemas; 
...
int OP_AD(ast::Node02* node, std::deque<lex::token02>::iterator &token) {
    if((*token).value == lex::Type::asValue(lex::Type::pattern::ADICAO) || (*token).value == lex::Type::asValue(lex::Type::pattern::SUBTRACAO)) {
        node->tokens.push_back(ast::Node02::pointer((*token)));
        token++;
        return SUCCESS;
...
int MAIS_FATORES(ast::Node02* node, std::deque<lex::token02>::iterator &token) {
    std::deque<lex::token02> lexemas; 
...
int OP_MUL(ast::Node02* node, std::deque<lex::token02>::iterator &token) {
    if((*token).value == lex::Type::asValue(lex::Type::pattern::MULTIPLICACAO) || (*token).value == lex::Type::asValue(lex::Type::pattern::DIVISAO)) {
        node->tokens.push_back(ast::Node02::pointer((*token)));
        token++;
        return SUCCESS;
